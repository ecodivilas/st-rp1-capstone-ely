...
const handleSubmit = (e) => {
    ...
    // userInputs.type_id = "Admin"
    // console.log(userInputs)
    createUser(userInputs).then(res => console.log("User created successfully!")).catch(err => console.log("Error: ", err))
}
    ...
    <>
      <form className="bg-slate-900 w-5/6 p-4 pt-6 mt-4" onSubmit={handleSubmit}>
        ...
        <div className="grid md:grid-cols-2 md:gap-6">
          ...
          <div className="relative z-0 w-full mb-6 group">
              <input onChange={handleOnChange} type="date" name="birthdate" id="birthdate" className="block py-2.5 px-0 w-full text-sm text-white dark:text-black bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" style={{filter: 'invert(1)'}} placeholder=" " required />
              <label className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Birthdate (Ex. 23/04/2023)</label>
          </div>
        </div>
        <button className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Create Account</button>
      </form>
    </>
  )
}
